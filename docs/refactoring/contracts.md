...
TX-01～05は、ユーザーによる現行差異の維持方針の確認後、[編集の確定と失敗](../../spec/13-編集の確定と失敗.md)へ正式仕様として集約した。この草案の比較表は調査資料であり、今後の変更は正式仕様を参照する。

所属・編集可否と履歴の本文はM2で[所属と編集可否](../../spec/contracts/所属と編集可否.md)と[編集と履歴](../../spec/contracts/編集と履歴.md)へ集約した。以下のOWN・REF・EDIT・HIST等の草案IDは比較用であり、正式な仕様IDとして採用したものではない。
...
TX-02とTX-03は操作単位で区別し、TX-03はドラッグやParameter適用の全体復元を上書きしない。低水準solve関数が成功を返しても、依存側の結果は別であり、呼出し側の確定成功を意味しない。呼出し経路全件の保証は引き続き検証対象とする。
...
戻り値やtransactionモデルは、この草案だけを根拠に新設しない。正式化したTX-02／03を各呼出し経路に対応付け、現在のsnapshot各種が保持する内容を比較する。
