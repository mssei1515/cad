...
内容整理とファイル移動を別に記録する。M1で移動済みの行には実在する参照先を記載した。移動案内として残した旧見出しも追跡対象に含む。複数IDは本文を分割する予定先を示し、同文の複写先ではない。小見出しのない段落は直前の見出しに含める。移動時は段落の振り分けを照合し、実在する移動先・見出しへのリンクと残範囲を記録する。削除時は集約先または承認された廃止理由を残す。
