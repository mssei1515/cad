...
async function createOpenSpline(page) {
  ...
  await expect.poll(() => page.evaluate(() => window.__cadTest.splineStateForTest().mode)).toBe("spline");
...
test("creates and edits a cubic fit-point spline and persists version 15", async ({ page }) => {
  ...
  await closed.check();
  expect((await page.evaluate(() => window.__cadTest.splineStateForTest())).direct[0].closed).toBe(true);
  await page.keyboard.press("Control+z");
  expect((await page.evaluate(() => window.__cadTest.splineStateForTest())).direct[0].closed).toBe(false);
  ...
  const fitPointClient = await page.evaluate((id) => window.__cadTest.geometryClientPositionForTest("point", id), fitPointId);
...
test("copies splines and projects them through blocks", async ({ page }) => {
  ...
  const serialized = await page.evaluate(() => window.__cadTest.splineStateForTest().serialized);
  ...
  expect(await page.evaluate((data) => window.__cadTest.loadDocumentFixtureForDragTest(data, "splines.cad2"), serialized)).toEqual(expect.objectContaining({ success: true }));
  expect((await page.evaluate(() => window.__cadTest.splineStateForTest())).direct).toHaveLength(2);
});

test("persists point-on-spline and endpoint tangent constraints", async ({ page }) => {
  await createOpenSpline(page);
  const state = await page.evaluate(() => window.__cadTest.exerciseSplineConstraintsForTest());
  ...
  expect(await page.evaluate((data) => window.__cadTest.loadDocumentFixtureForDragTest(data, "spline-constraints.cad2"), state.serialized)).toEqual(expect.objectContaining({ success: true }));
  expect((await page.evaluate(() => window.__cadTest.exerciseSplineConstraintsForTest())).types).toEqual(expect.arrayContaining(["pointOnSpline", "splineLineTangent"]));
});

test("rebuilds the hatch preview cache after spline fit points move", async ({ page }) => {
  const state = await page.evaluate(() => window.__cadTest.splineHatchPreviewCacheForTest());
...
test("migrates version 14 without splines and rejects malformed version 15 spline data", async ({ page }) => {
  ...
  expect(await page.evaluate((data) => window.__cadTest.loadDocumentFixtureForDragTest(data, "malformed-spline.cad2"), malformed)).toEqual(expect.objectContaining({ success: false }));
  expect((await page.evaluate(() => window.__cadTest.splineStateForTest())).direct).toHaveLength(1);
  ...
  expect(await page.evaluate((data) => window.__cadTest.loadDocumentFixtureForDragTest(data, "cross-sketch-spline.cad2"), crossSketch)).toEqual(expect.objectContaining({ success: false }));
  expect((await page.evaluate(() => window.__cadTest.splineStateForTest())).direct).toHaveLength(1);
  ...
  expect(await page.evaluate((data) => window.__cadTest.loadDocumentFixtureForDragTest(data, "legacy-v14.cad2"), legacy)).toEqual(expect.objectContaining({ success: true }));
  expect((await page.evaluate(() => window.__cadTest.splineStateForTest())).direct).toHaveLength(0);
});
