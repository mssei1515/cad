...
Projection Geometryの外観継承は[表示と注記](./06-表示と注記.md)のAPP-03に従う。配置先SketchとDefinition内Sketchの外観を区別し、Instance Overrideを最後に適用する。
...
- 無効化されるProjectionへの参照は[削除と参照](./14-削除と参照.md)の「Blockの構成変更」に従って扱う。
...
失敗時の復元範囲と履歴は[編集の確定と失敗](./13-編集の確定と失敗.md)に従う。Block Parameter適用にはTX-04、Parameter以外のDefinition編集による外部拘束エラーにはTX-05を適用する。
...
子Definition編集によるProjection削除の扱いは[削除と参照](./14-削除と参照.md)の「Blockの構成変更」に従う。参照先を失った内部Constraintが保存データに残る場合の読込は、[データモデルと永続化](./02-データモデルと永続化.md)のLOAD-03に従う。

## 14. 削除と互換性

- Definition・Instanceの削除可否と参照整理は[削除と参照](./14-削除と参照.md)に従う。
