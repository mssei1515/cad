...
保存時はpathを`@`で連結するため、従来どおり`BI1@BI2@L1`となる。Constraintの保存fieldも同じcodecで生成・解決するが、保存文字列にはkindを追加せず従来のbare IDを維持する。Presentationのstyleと注記参照は`line:BI1@BI2@L1`のようなtyped keyを使い、いずれも同じresolver契約でProjectionを解決する。
