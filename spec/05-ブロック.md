...
- 無効化されるProjectionへの参照は[Blockの構成変更](./operations/Block.md#1-blockの構成変更)に従って扱う。
...
子Definition編集によるProjection削除の扱いは[Blockの構成変更](./operations/Block.md#1-blockの構成変更)に従う。参照先を失った内部Constraintが保存データに残る場合の読込は、[データモデルと永続化](./02-データモデルと永続化.md)のLOAD-03に従う。
