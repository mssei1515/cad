...
- ユーザー向けPropertiesではInstanceを単に「ブロック」と表記し、Instance ID、Block Definition名とID、`x`、`y`、角度、回転モード、表示する内部 Sketch を表示する。`x`、`y`、角度は他のGeometryの測定値と同様に読み取り専用とし、Propertiesから直接変更しない。選択済みInstanceの回転モードと表示内部SketchはPropertiesで変更し、Explorer下部には表示しない。Explorer下部の回転・内部Sketch設定は新規配置中だけ表示する。
