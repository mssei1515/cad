...
- Parameterの参照寸法feedbackはドラッグ中に反復しない。
- Pointer-upで最終精度のsolveとParameter再評価を行う。
- 式エラー、拘束矛盾、非収束時はドラッグ開始前へ戻す。
...
1. 参照寸法を測定する。
2. 式を依存順に評価する。
3. 拘束寸法の`target`を更新する。
4. 拘束をsolveする。
5. 参照寸法を再測定する。

各参照値の変化が`1e-7 × max(1, |value|)`以内で、solverも成功した場合に収束とする。式エラー、拘束矛盾、非収束では操作全体を開始前へロールバックし、履歴へ追加しない。
