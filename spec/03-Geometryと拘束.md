...
通常GeometryはAppearanceで解決した色、線種、線幅、visibleを使用する。初期の基底線幅は2.0pxであり、ユーザーはRoot SketchのAppearanceから通常Geometryの既定を設定する。補助GeometryはRoot SketchのPropertiesにあるDefault Construction Appearanceを適用し、初期値は一点鎖線、1.1px、通常時の不透明度72%とする。その後にSketchまたはGeometryの明示Appearanceを適用する。補助LineのendpointOverhangがtrueなら両端から画面上12pxずつ延長し、endpointMarkersがtrueなら元の両端へ点を描画する。Geometry自身または所属Block Instanceのhover／選択中も設定したはみ出しを維持し、Constraint Status ViewではendpointMarkersの値にかかわらず端部の点を表示しない。選択とhoverは一時的な強調を優先し、Space押下中は拘束状態色を優先する。
...
Parameterを含む確定操作では、参照寸法の測定、式の依存順評価、拘束寸法`target`更新、拘束solve、再測定を最大20回反復する。各参照値の変化が`1e-7 × max(1, |value|)`以内で、solverも成功した場合に収束とする。式エラー、拘束矛盾、非収束では操作全体を開始前へロールバックし、履歴へ追加しない。
