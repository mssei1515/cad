...
- 派生Instanceの生成Geometryはsnap、拘束、寸法、Hatch、Leaderに使用できる。通常選択はInstance単位、拘束・寸法コマンドでは生成Geometry単位とする。生成Geometryのdragは派生変換を逆変換して最終的な参照元Geometryの通常dragとして処理し、参照元の拘束を維持する。trim、fillet、通過Point追加・削除などのSpline構造編集、固定、通常／補助作図切替は派生側で拒否する。AppearanceはInstanceの上書きとして編集できる。
