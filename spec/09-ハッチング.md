...
単体テストは矩形、未分割交差、Circle穴、Line／Arc／Spline混在、閉Spline、隣接面、開放境界、重複区間、境界追従、欠落無効化、参照書換え、preview index再利用を検証する。E2Eは作成preview、継続command、Properties、Tree、Color、平行線・クロス・solid描画と不透明度、screen固定間隔、Undo/Redo、version 12移行、version 17往復・opacity省略補完・不正pattern／opacity読込の原子性、境界再指定、Copy／Block化の完全境界要求、入れ子Block変換、Appearance Override、hit testを検証する。
