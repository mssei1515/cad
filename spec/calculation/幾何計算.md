...
- `normalizeAngleSigned`は角度を`(-π, π]`へ正規化し、Solverの角度残差で使う。境界の`-π`は`π`になる。
  周回数に比例した繰返しを使わず、非有限の入力は`NaN`を返す。
...
有効な拘束が端点を参照するArcでは、表示上の最小sweepへの補正で解いた端点角度を上書きしない。読込時も拘束を復元してからこの判定を行う。
...
単一Lineの`OffsetConstraint`は、元Lineと結果Lineの平行および元Lineからの符号付き支持線距離だけを残差とする。結果Lineの始点・終点の長さ方向位置と長さは拘束せず、両端を伸縮できる。複数要素の`OffsetChainConstraint`はこの扱いとは別で、各結果Geometryの距離・方向に加えて隣接端のマイター接続を拘束する。開チェーンの先頭・末尾ではこの端部位置拘束を追加せず、非接続端の伸縮を許可する。閉チェーンおよびチェーン内部の接続端は従来どおり接続を維持する。
