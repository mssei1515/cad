...
Document adapter、Geometry command、Constraint command、Block service、Parameter service、Appearance resolver、Hatch、Annotation、Canvas renderer、DOM panel、test fixtureを段階的にmoduleへ分ける。分割中もversion 15 schemaと既存GeometryRefを維持する。
...
version 15 JSON、Appearance、Dimension Appearance、Spline、Hatch、Annotation、チェーンオフセット参照に包括的なschema validationを導入し、不正値の補正と拒否条件を明示する。Parameter名前空間の式・依存検証とAnnotation所属・Leader参照検証は現行Loaderで先行して実施する。
