...
Document adapter、Geometry command、Constraint command、Block service、Parameter service、Appearance resolver、Hatch、Annotation、Reference Image、Sketch Projection、Canvas renderer、DOM panel、test fixtureを段階的にmoduleへ分ける。分割中もversion 20 schemaと既存GeometryRefを維持する。
...
version 20 JSON、Appearance、Dimension Appearance、Spline、Hatch、Annotation、Reference Image、Sketch Projection、中心線拘束、チェーンオフセット参照に包括的なschema validationを導入し、不正値の補正と拒否条件を明示する。Parameter名前空間の式・依存検証、Annotation所属・Leader参照検証、Reference Imageの構造・所属検証は現行Loaderで先行して実施する。
