...
寸法値の表示保証精度は、線形寸法では `0.000001 mm`、角度寸法では `0.000001°` とする。

表示時はこの精度へ丸め、ソルバー計算や浮動小数点演算で生じた保証精度未満の微小な誤差を表示しない。

小数部がゼロになる値は整数として表示し、小数部がある値は末尾の不要なゼロを省略する。

この規則は、Geometry の拘束寸法、読み取り専用寸法、Presentation の注記寸法に共通して適用する。
