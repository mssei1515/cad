...
仕様整理の検討資料は [仕様整理と段階的リファクタリング](../docs/refactoring/README.md) を参照する。草案・移行候補・未決定事項は正式仕様ではない。7領域への[移行先と進捗](../docs/refactoring/migration-plan.md)を別途管理している。以下は現在の正式文書一覧であり、再配置はまだ行っていない。
