...
JSON version 16では、version 10で導入したParameterの意味を維持し、`target`を評価済みcacheとして残して拘束寸法の`expression`を正式な入力値とする。チェーンオフセット寸法もチェーン全体で1つの寸法symbolと式を持つ。Document、Block Definition、History、Block Editor draft、clipboardへParameter、寸法名、式、採番counterを保存する。
